...
task format_taxa {
    ...
    input {
        File taxa_file
        String? sptaxa_file="sptaxa_file.tsv"
    }

    command <<<
        i=0
        echo -e "taxid\tsegment\trefseqs\tsptaxa" >> "~{sptaxa_file}"
        while read -r taxonomy; do
            ...
            echo -e "$taxid\t$segment\t$ref_accs\t$sptaxa" >> "~{sptaxa_file}"
        done < <(tail -n +2 ~{taxa_file})
    >>>
    
    output {
        Array[Object] taxa=read_objects("~{sptaxa_file}")
...
task parallel_adapt {
    ...
    command <<<
        ~{base_cmd} ~{args_specificity} ~{specificity_taxa} ~{args_obj} ~{args_influenza}
    >>>
...
workflow wf {
    ...
    scatter(sp in sps) {
        scatter(obj in objs) {
            scatter(i in range(length(format_taxa.taxa))) {
                Object taxon = format_taxa.taxa[i]
                call parallel_adapt {
                    input: 
                        taxid = taxon["taxid"],
