...
task adapt {
    ...
    String base_cmd = "design.py complete-targets auto-from-args ~{taxid} '~{segment}' ~{taxid}_~{segment}_~{sp_str}_guides -gl ~{gl} -pl ~{pl} -pm ~{pm} -pp ~{pp} --primer-gc-content-bounds ~{primer_gc_lo} ~{primer_gc_hi} --max-primers-at-site ~{max_primers_at_site} --max-target-length ~{max_target_length} --obj-fn-weights ~{objfnweights_a} ~{objfnweights_b} --best-n-targets ~{bestntargets} --predict-cas13a-activity-model --mafft-path $MAFFT_PATH --cluster-threshold ~{cluster_threshold} --write-annotation ~{taxid}_~{segment}_~{sp_str}_annotation"
    ...
    String args_aln = if write_aln then " --write-input-aln ~{taxid}_~{segment}_~{sp_str}_alignment" else ""
...
workflow parallel_adapt {
    ...
    output {
        ...
        Array[Array[Array[Array[File]]]] anns = adapt.anns
